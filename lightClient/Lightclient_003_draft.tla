...
\*   IG:  2/3 of all verifiers are signing this commit.
\* Q: Why not use here the twoThirds predicate from Blockchain_003_draft?
...
(** @type: (BLOCK, BLOCK) => Bool;
  ...
  * in a trusted header.
 *)
\*  IG: Because this then implies that at least one of the verifiers is trusted (since we assume
\*  that more than 2/3 are trusted, therefore, less than 1/3 are faulty).
\* Q: There was also a similar predicate in the blockchain_003. Why not use ethat?
...
\* Q: what is the difference between the properties listed here and the ones in the file LightTests.tla
